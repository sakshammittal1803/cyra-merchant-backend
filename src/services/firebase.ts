...
export async function listenToFirebaseOrders(callback: (order: any) => void) {
  ...
  try {
    ...
    setInterval(async () => {
      try {
        ...
        const response = await axios.get(`${FIREBASE_DB_URL}/orders.json`);
        
        if (response.data) {
          const allOrders = Object.entries(response.data);
          const pendingOrders = allOrders.filter(([_, order]: [string, any]) => order.status === 'pending');
          ...
          pendingOrders.forEach(([key, order]: [string, any]) => {
...
export async function updateFirebaseOrderStatus(firebaseKey: string, status: string) {
  try {
    await axios.patch(
      `${FIREBASE_DB_URL}/orders/${firebaseKey}.json`,
      { status, merchantUpdatedAt: Date.now() }
    );
...
export async function syncMenuToFirebase(menuItems: any[]) {
  try {
    ...
    await axios.put(
      `${FIREBASE_DB_URL}/merchantMenu.json`,
      menuData
    );
...
export async function getFirebaseMenu() {
  try {
    const response = await axios.get(`${FIREBASE_DB_URL}/merchantMenu.json`);
...
export function convertFirebaseOrderToMerchant(firebaseOrder: any) {
  return {
    ...
    items: (firebaseOrder.items || []).map((item: any) => ({
      item_name: item.name,
